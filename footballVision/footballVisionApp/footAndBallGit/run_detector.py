...
from .network.footandball import FootAndBall
from .data.augmentation import numpy2tensor
...
def run_detector(model: FootAndBall, args: argparse.Namespace):
    print(args)
    print(os.path.getsize(args.path))
    model.print_summary(show_architecture=False)
    model = model.to(args.device)

    _, file_name = os.path.split(args.path)
    print(os.path.getsize(args.weights))
...
def main(path, outPath):
    if path and outPath:
        print("Main correct")
        Model = model_factory('fb1', 'detect', ball_threshold=0.7, player_threshold=0.7)
        # time.sleep(3)
        # Create argparse.Namespace object with the required arguments
        args = argparse.Namespace(
            path=f"D:/Polytech/5A/Django/footballVision/media/uploads/{path}",
            model='fb1',
            weights="D:/Polytech/5A/Django/footballVision/footballVisionApp/footAndBallGit/models/model_20201019_1416_final.pth",
            ball_threshold=0.7,
            player_threshold=0.7,
            out_video=outPath,
            device='cpu'  # You might want to adjust the device based on your system
        )
...
if __name__ == '__main__':
    ...
    model = model_factory(args.model, 'detect', ball_threshold=args.ball_threshold,
